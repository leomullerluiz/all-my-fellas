...
/**
 * Starts the highest-priority eligible `on_queue` task, if a slot is free.
 *
 ...
 * poller. This is also the mechanism behind "dependencies are considered
 * while a task sits in the On Queue column" (`stories.md` S2): a
 * dependency-blocked candidate is skipped in favour of the next eligible one
 * rather than leaving a free slot idle while another queued task could run.
 *
 * A `CapacityError` stops the loop outright — every remaining candidate would
 * fail the same admission check, so trying them is wasted work. A
 * `DependencyError`, `InvalidTransitionError` (the task was started
 * elsewhere in the same instant) or `TaskNotFoundError` (cancelled/deleted
 * meanwhile) is specific to that one candidate, so the loop moves on to the
 * next. Anything else is an unanticipated failure that would otherwise
 * strand a task at `on_queue` with zero signal, so it is logged, and the
 * loop still moves on rather than leaving the rest of the queue stalled.
 *
...
export function promoteQueue(): void {
  const candidates = sortByPriorityThenDifficulty(queuedTasks());
